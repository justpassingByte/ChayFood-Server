...
exports.getRegionalOrders = exports.getOrderTrends = exports.getPopularDishes = exports.getCustomerStats = exports.getOrderStats = void 0;
...
const getRegionFromAddress = (state) => {
    ...
    if (northRegions.some(r => state.includes(r)))
        return 'North';
    if (centralRegions.some(r => state.includes(r)))
        return 'Central';
    if (southRegions.some(r => state.includes(r)))
        return 'South';
    return 'Other';
};
...
const getOrderStats = async (req, res) => {
    try {
        ...
        if (region && region !== 'all') {
            ...
            const usersInRegion = await User_1.User.find({
                'addresses.state': { $regex: new RegExp(region, 'i') }
            }).select('_id');
        ...
        if (category && category !== 'all') {
            ...
            const menuItemsInCategory = await MenuItem_1.MenuItem.find({
                category: category
            }).select('_id');
        ...
        const [currentOrders, previousOrders, cancelledOrders] = await Promise.all([
            Order_1.Order.find(Object.assign({}, query)).exec(),
            Order_1.Order.find(Object.assign({}, prevQuery)).exec(),
            Order_1.Order.find({
                createdAt: { $gte: start, $lte: end },
                status: 'cancelled'
            }).exec()
        ...
        const completedOrders = currentOrders.filter(o => ['delivered', 'ready'].includes(o.status)).length;
...
const getCustomerStats = async (req, res) => {
    try {
        ...
        const allUsers = await User_1.User.find(regionQuery).select('_id createdAt');
        // Get users created in current period (new users)
        const newUsers = allUsers.filter(user => user.createdAt >= start && user.createdAt <= end);
        // Get users created in previous period
        const prevNewUsers = allUsers.filter(user => user.createdAt >= prevStart && user.createdAt <= prevEnd);
        // Find users who placed orders in current period
        const ordersInPeriod = await Order_1.Order.find({
            createdAt: { $gte: start, $lte: end }
        }).select('user').distinct('user');
        // Find users who placed orders in previous period
        const ordersInPrevPeriod = await Order_1.Order.find({
            createdAt: { $gte: prevStart, $lte: prevEnd }
        }).select('user').distinct('user');
...
const getPopularDishes = async (req, res) => {
    try {
        ...
        let menuItemQuery = {};
        if (category && category !== 'all') {
            menuItemQuery = { category: category };
        }
        // Get all menu items
        const menuItems = await MenuItem_1.MenuItem.find(menuItemQuery).select('_id name price category');
        ...
        orders.forEach(order => {
            order.items.forEach(item => {
                const menuItemId = item.menuItem instanceof mongoose_1.default.Types.ObjectId
                ...
                if (!dishPopularity[menuItemId]) {
                    dishPopularity[menuItemId] = {
                        count: 0,
                        revenue: 0,
                        name: item.menuItem.name || menuItem.name
                    };
                }
...
exports.getPopularDishes = getPopularDishes;
/**
 * Get order trends
 */
 ...
 * Get regional orders
 */
const getRegionalOrders = async (req, res) => {
    try {
        // Extract filter parameters
        ...
        orders.forEach(order => {
            ...
            if (order.deliveryAddress && order.deliveryAddress.state) {
                ...
                const defaultAddress = order.user.addresses.find((addr) => addr.isDefault) ||
...
exports.getRegionalOrders = getRegionalOrders;

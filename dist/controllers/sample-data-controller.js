...
exports.clearSampleData = exports.generateAll = exports.generateOrders = exports.generateUsers = exports.generateMenuItems = void 0;
...
const randomChoiceWeighted = (choices) => {
...
const generateMenuItems = async (req, res) => {
    try {
        ...
        await MenuItem_1.MenuItem.insertMany(menuItems);
...
exports.generateMenuItems = generateMenuItems;
/**
...
const generateOrders = async (req, res) => {
    try {
        ...
        const users = await User_1.User.find({}).select('_id addresses');
        const menuItems = await MenuItem_1.MenuItem.find({}).select('_id price category');
        ...
        for (let i = 0; i < count; i++) {
            ...
            const regionUserAddress = (regionUser.addresses && regionUser.addresses.length > 0)
                ? (regionUser.addresses.find((a) => a.isDefault) || regionUser.addresses[0])
                : null;
            if (!regionUserAddress)
                continue;
            ...
            for (let j = 0; j < itemCount; j++) {
                ...
                const categoryItems = menuItems.filter((item) => item.category === categoryType);
...
const generateAll = async (req, res) => {
    try {
        const { menuItems = 20, users = 50, orders = 200, timeRange = {
        ...
        const menuReq = { body: { count: menuItems } };
        await (0, exports.generateMenuItems)(menuReq, res);
        // 2. Generate users
        await User_1.User.deleteMany({});
        const userReq = { body: { count: users } };
        await (0, exports.generateUsers)(userReq, res);
        // 3. Generate orders
        ...
        await (0, exports.generateOrders)(orderReq, res);
        return res.status(201).json({
...
exports.clearSampleData = clearSampleData;

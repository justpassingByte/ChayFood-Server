...
exports.reorderPreviousOrder = reorderPreviousOrder;
...
async function getAllOrders(req, res) {
    ...
    try {
        ...
        if (!req.user || !((_c = req.user) === null || _c === void 0 ? void 0 : _c._id)) {
...
async function getUserOrders(req, res) {
    ...
    try {
        ...
        console.log(`Fetching orders for user: ${(_c = req.user) === null || _c === void 0 ? void 0 : _c._id}`);
        const orders = await Order_1.Order.find({ user: (_d = req.user) === null || _d === void 0 ? void 0 : _d._id })
            .populate('items.menuItem');
...
async function getOrderById(req, res) {
    ...
    try {
        ...
        if (((_d = req.user) === null || _d === void 0 ? void 0 : _d.role) !== 'admin' && order.user.toString() !== ((_e = req.user) === null || _e === void 0 ? void 0 : _e._id.toString())) {
...
async function createOrder(req, res) {
    ...
    try {
        ...
        try {
            await (0, loyalty_service_1.awardPointsForOrder)(req.user._id, order);
...
async function updateOrderStatus(req, res) {
    ...
    try {
        ...
        else if (statusHierarchy[currentOrder.status] >= statusHierarchy['ready'] &&
            statusHierarchy[req.body.status] < statusHierarchy[currentOrder.status]) {
            res.status(400).json({
                status: 'error',
 ...
 */
async function reorderPreviousOrder(req, res) {
    var _a;
    try {
        // Check if user is authenticated
        if (!req.user || !((_a = req.user) === null || _a === void 0 ? void 0 : _a._id)) {
        ...
        const originalOrder = await Order_1.Order.findById(orderId)
            .populate('items.menuItem');
        if (!originalOrder) {
            res.status(404).json({
                status: 'error',
        ...
        for (const item of originalOrder.items) {
            ...
            newItems.push({
                menuItem: item.menuItem,
                quantity: item.quantity,
                price: item.menuItem.price, // Get current price
                specialInstructions: item.specialInstructions
            });
        ...
        // Award loyalty points for the order
        try {
            await (0, loyalty_service_1.awardPointsForOrder)(req.user._id, newOrder);
        }
        catch (pointsError) {
            console.error('Error awarding loyalty points:', pointsError);
            // Don't fail the order if points can't be awarded
...
async function getOrderBySessionId(req, res) {
    ...
    try {
        ...
        const order = await Order_1.Order.findOne({ stripeSessionId: sessionId }).populate('items.menuItem');
        if (!order) {
            console.log('No order found with stripeSessionId:', sessionId);
            // For debugging: Check if any orders have stripeSessionId field
            const allOrders = await Order_1.Order.find({ stripeSessionId: { $exists: true } });
            ...
            try {
                ...
                if (session && session.payment_status === 'paid') {
                    ...
                    if (!((_a = session.metadata) === null || _a === void 0 ? void 0 : _a.items)) {
                    ...
                    try {
                        ...
                        else if ((_b = session.customer_details) === null || _b === void 0 ? void 0 : _b.email) {
                        ...
                        const populatedOrder = await Order_1.Order.findById(newOrder._id).populate('items.menuItem');

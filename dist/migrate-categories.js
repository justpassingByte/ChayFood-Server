...
const OldMenuItemSchema = new mongoose_1.default.Schema({
    name: String,
    description: String,
    price: Number,
    category: {
...
const OldMenuItem = mongoose_1.default.model('OldMenuItem', OldMenuItemSchema);
...
async function migrateCategories() {
    try {
        ...
        for (const categoryData of defaultCategories) {
            ...
            const existingCategory = await Category_1.Category.findOne({ slug: categoryData.slug });
            if (existingCategory) {
        ...
        for (const menuItem of menuItems) {
            ...
            await mongoose_1.default.connection.collection('menuitems').updateOne({ _id: menuItem._id }, { $set: { category: categoryId } });
            updatedCount++;
